    ...
    public void sendEmailChangeCode(String email, String code) {
        try {
            CreateEmailOptions request = CreateEmailOptions.builder()
                    .from(fromEmail)
                    .to(email)
                    .subject("Codigo de Confirmacao para Troca de Email")
                    .html(buildEmailChangeCodeHtml(code))
                    .build();

            CreateEmailResponse response = resend.emails().send(request);
            log.info("Email de confirmacao de troca de email enviado para {} com id {}", email, response.getId());
        } catch (Exception e) {
            log.error("Erro ao enviar email de confirmacao de troca de email para {}", email, e);
        }
    }

    private String buildEmailChangeCodeHtml(String code) {
        return """
                <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
                  <h2 style="color: #1f2937;">Confirmacao de Troca de Email</h2>
                  <p>Use o codigo abaixo para confirmar a troca do seu email de acesso:</p>
                  <div style="font-size: 32px; font-weight: bold; letter-spacing: 8px; padding: 16px;
                              background: #f3f4f6; border-radius: 8px; text-align: center; color: #4f46e5;">
                    %s
                  </div>
